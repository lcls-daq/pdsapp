...
void ConfigSelect::_reconfigure()
{
  ...
  for(SList::const_iterator it = _servers.begin(); it != _servers.end(); it++) {
    ...
    { Pds::Sockaddr sa(local);
      if (::bind(s, sa.name(), sa.sizeofName()) < 0)
    ...
    if (!hent) 
      ...
      Pds::Ins ins(ip_host, it->port);
      {

	{ Pds::Sockaddr sa(ins);
	  if (::connect(s, sa.name(), sa.sizeofName())<0)
