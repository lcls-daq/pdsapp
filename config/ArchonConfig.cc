...
class Pds_ConfigDb::ArchonConfig::Private_Data {
 ...
 public:
  ...
  ~Private_Data();
   ...
   int push( void* to );
   int dataSize() const
      { return sizeof(ArchonConfigType); }
...
const char* Pds_ConfigDb::ArchonConfig::Private_Data::lsEnumReadoutMode[] = { "Single", "Continuous", "Triggered", NULL};

Pds_ConfigDb::ArchonConfig::Private_Data::Private_Data() :
  _enumReadoutMode    ("Readout Mode",          ArchonConfigType::Single, lsEnumReadoutMode),
...
{
  _conf = new TextParameter("Configuration File", "", ArchonConfigType::FILENAME_CHAR_MAX);
}
...
void Pds_ConfigDb::ArchonConfig::Private_Data::insert( Pds::LinkedList<Parameter>& pList )
{
...
int Pds_ConfigDb::ArchonConfig::Private_Data::pull( void* from )
{
  ...
  strncpy(_conf->value, cfg.config(), ArchonConfigType::FILENAME_CHAR_MAX);
  return sizeof(ArchonConfigType);
}
  
int Pds_ConfigDb::ArchonConfig::Private_Data::push(void* to)
{
  char    cm[ArchonConfigType::FILENAME_CHAR_MAX];
  strncpy(cm, _conf->value, ArchonConfigType::FILENAME_CHAR_MAX);
  
  new (to) ArchonConfigType(
...
Pds_ConfigDb::ArchonConfig::ArchonConfig() :
