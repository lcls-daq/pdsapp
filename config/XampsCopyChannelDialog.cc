...
namespace Pds_ConfigDb {
      ...
      : QDialog(parent), index(ind)
  {
    ...
    sprintf(foo, "XAMPS Copy Channel %d", index);
  ...
  void XampsCopyChannelDialog::copyClicked()
  {
    XampsChannelData* src = _config->_asic[myAsic]->_channel[index];
    unsigned cb = 0;
    for (int i=0; i<numberOfChannels; i++) {
      if (i != index) {
        if (channelCheckBox[cb]->checkState() == Qt::Checked) {
          XampsChannelData* dest = _config->_asic[myAsic]->_channel[i];
          for (int j=0; j<XampsChannel::NumberOfChannelBitFields; j++) {
  ...
  void XampsCopyChannelDialog::selectNoneClicked()
  {
    unsigned cb = 0;
    for (int i=0; i<numberOfChannels; i++)
    {
