...
namespace Pds_ConfigDb
{
  ...
  public:
    ...
    QCheckBox*            _outputs[EvrConfigType::EvrOutputs];
  };
};
...
void EvrPulseTable::pull(const EvrConfigType& tc) {
  ...
  for(unsigned j=0; j<tc.npulses(); j++) {
    ...
    bool lUsed=false;
    for(unsigned k=0; k<EvrConfigType::EvrOutputs; k++)
    ...
    for(unsigned j=0; j<EvrConfigType::EvrOutputs; j++) {
      if (p._outputs[j]->isChecked())
        *new(&om[nom++]) EvrConfigType::OutputMapType( EvrConfigType::OutputMapType::Pulse, npt+p0,
                                                       EvrConfigType::OutputMapType::UnivIO, j+EvrConfigType::EvrOutputs*_id );
    }
    npt++;
  }
...
QLayout* EvrPulseTable::initialize(QWidget*) 
{
  ...
  while(j < EvrConfigType::EvrOutputs) {
...
{
  int row = k/EvrConfigType::EvrOutputs;
  int col = k%EvrConfigType::EvrOutputs;
