...
QLayout* Pds_ConfigDb::ZylaConfig::Private_Data::initialize(QWidget* p)
{
  ...
  { QVBoxLayout* d = new QVBoxLayout;
    ...
    d->addLayout(_exposureTime  .initialize(p));
...
Pds_ConfigDb::ZylaConfig::ZylaConfig() :
...
class Pds_ConfigDb::ZylaExpertConfig::Private_Data : public Parameter {
  static const char*  lsEnumATBool[];
...
QLayout* Pds_ConfigDb::ZylaExpertConfig::Private_Data::initialize(QWidget* p)
{
  QVBoxLayout* layout = new QVBoxLayout;
  { QVBoxLayout* m = new QVBoxLayout;
    m->addWidget(new QLabel("ROI / Image settings: "));
  ...
  { QVBoxLayout* d = new QVBoxLayout;
    d->addWidget(new QLabel("Trigger / Readout settings: "));
    d->addLayout(_shutter       .initialize(p));
    d->addLayout(_gainMode      .initialize(p));
    d->addLayout(_readoutRate   .initialize(p));
    d->addLayout(_overlap       .initialize(p));
    d->addLayout(_boxTrigger    .initialize(p));
...
int Pds_ConfigDb::ZylaExpertConfig::Private_Data::pull( void* from )
{
  ZylaConfigType& cfg = * new (from) ZylaConfigType;
  _cooling.value            = cfg.cooling();
...
int Pds_ConfigDb::ZylaExpertConfig::Private_Data::push(void* to)
{
  if (_boxROI.value) {
    // using a centered ROI
    _orgX.value = (CAMERA_MAX_WIDTH  - _width.value ) / 2 + 1;
...
Pds_ConfigDb::ZylaExpertConfig::ZylaExpertConfig() :
  Serializer("ZylaExpertConfig"),
  _private_data( new Private_Data )
{
  pList.insert(_private_data);
}

int Pds_ConfigDb::ZylaExpertConfig::readParameters (void* from)
{
  return _private_data->pull(from);
}

int Pds_ConfigDb::ZylaExpertConfig::writeParameters(void* to)
{
  return _private_data->push(to);
}

int Pds_ConfigDb::ZylaExpertConfig::dataSize() const
{
  return _private_data->dataSize();
}
