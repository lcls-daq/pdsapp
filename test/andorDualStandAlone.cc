...
static int iCameraInitialized[] = {0, 0};
...
private:
  int _initCamera(int iCamera);
  int _selectCamera(int iCamera);
  int _prepAcquisition(int iCamera);
  int _retrieveData(int iCamera, uint16_t* liImageData, int iImageWidth, int iImageHeight,
                    const unsigned char uTestByte, const int iTestBufferSizeIn16,  bool bExtTrigger);
...
AndorCameraTest::AndorCameraTest(int iCamera, double fExposureTime, int iReadoutPort, int iSpeedIndex,
  ...
  _iCamera(iCamera), _iCameraMaster(-1), _iCameraSlave(-1), _fExposureTime(fExposureTime),
  _iReadoutPort(iReadoutPort), _iSpeedIndex(iSpeedIndex), _iGainIndex(iGainIndex), _fTemperature(fTemperature),
  _iRoiX(iRoiX), _iRoiY(iRoiY), _iRoiW(iRoiW), _iRoiH(iRoiH), _iBinX(iBinX), _iBinY(iBinY),
  _iTriggerMode(iTriggerMode), _strFnPrefix(sFnPrefix), _iNumImages(iNumImages), _iMenu(iMenu), _bTriggerFast(bTriggerFast),
...
int AndorCameraTest::init()
{
  ...
  if (_iCamera > iMaxCamera)
  {
    printf("Number of selected %d cameras exceedes maximum of %d\n", _iCamera, iMaxCamera);
    return 1;
  }

  for (int iCamera=0; iCamera<_iCamera; ++iCamera)
  {
    iError |= (_initCamera(iCamera)<<(4*iCamera));
  }

  return iError;
}

int AndorCameraTest::_initCamera(int iCamera)
{
  unsigned iError;
  ...
  //Determine if the camera is a master or slave
  if (iSerialNumber != 0)
  {
    _iCameraMaster = iCamera;
    printf("Camera is master\n");
  }
  else
  {
    _iCameraSlave = iCamera;
    printf("Camera is slave\n");
  }
...
int AndorCameraTest::_selectCamera(int iCamera)
{
  ...
  printf("Switched to Andor Camera: %d\n", iCamera);
...
int AndorCameraTest::_prepAcquisition(int iCamera)
{
  int iError;
  ...
  if (iError != 0)
  {
    printf("Failed to switch to camera: %d\n", iCamera);
    return 1;
  }
  ...
  printf(">> Acquisition Setup Time %6.1lf  Preparation Time %6.1lf ms -- (camera %d)\n", fTimeSetupAcq, fTimePreAcq, iCamera);
...
int AndorCameraTest::_runAcquisition()
{
  ...
  for (int iCamera=0; iCamera<_iCamera; ++iCamera)
  {
    iError = _prepAcquisition(iCamera);
    if (iError != 0)
    {
      printf("prepAcquistion failed for camera %d with error code: %d\n", iCamera, iError);
      return iError;
    }
  }

  // get readout time of the current node
  float fTimeReadout = -1;
  GetReadOutTime(&fTimeReadout);
  printf("Readout time: %f s\n", fTimeReadout);
  ...
  if (bExtTrigger)
  {
    iError = _selectCamera(_iCameraSlave);
    if (iError !=0)
      printf("Unable to select slave camera (cam %d)\n", _iCameraSlave);
    else
    {
      iError = StartAcquisition();
      if (!isAndorFuncOk(iError))
        printf("StartAcquisition() - slave: %s\n", AndorErrorCodes::name(iError));
    }

    timeval timeSleepMicro = {0, iAcquisitionDelayTime * 1000}; // in milliseconds
    // use select() to simulate nanosleep(), because experimentally select() controls the sleeping time more precisely
    select( 0, NULL, NULL, NULL, &timeSleepMicro);

    iError = _selectCamera(_iCameraMaster);
    if (iError !=0)
      printf("Unable to select slave camera (cam %d)\n", _iCameraMaster);
    else
    {
      iError = StartAcquisition();
      if (!isAndorFuncOk(iError))
        printf("StartAcquisition() - master: %s\n", AndorErrorCodes::name(iError));
    }
  }
  ...
  double fTimePrepSetupAcq  = (timeVala1.tv_nsec - timeVala0.tv_nsec) * 1.e-6 + ( timeVala1.tv_sec - timeVala0.tv_sec ) * 1.e3;
  printf(">> Acquisition Preparation and Setup Time %6.1lf  ms -- (Total)\n", fTimePrepSetupAcq);
  ...
  for (int iImage = 0; iImage < _iNumImages; ++iImage)
  {
    ...
    if (!bExtTrigger)
    {
      ...
      iError = _selectCamera(_iCameraMaster);
      if (iError !=0)
        printf("Unable to select master camera (cam %d)\n", _iCameraMaster);
      else
      {
        iError = StartAcquisition();
        if (!isAndorFuncOk(iError))
    ...
    if (!bExtTrigger)
    {
      ...
      if (iError != 0)
      {   
        printf("Unable to select slave master (cam %d)\n", _iCameraMaster);
        break;
      }
      iError = WaitForAcquisitionTimeOut(iMaxReadoutTime);
      if (!isAndorFuncOk(iError))
    ...
    else
    {
      while (numAcqNew == numAcq)
      {
        ...
        // DOES THIS NEED TO BE DONE FOR BOTH?
    ...
    iError = _retrieveData(_iCameraSlave, liImageData, iImageWidth, iImageHeight, uTestByte, iTestBufferSizeIn16, bExtTrigger);
    if (iError != 0)
    {
      printf("Failed to read data from slave (camera: %d)\n", _iCameraSlave);
      return 1;
    }

    iError = _retrieveData(_iCameraMaster, liImageData, iImageWidth, iImageHeight, uTestByte, iTestBufferSizeIn16, bExtTrigger);
    if (iError != 0)
    {
      printf("Failed to read data from master (camera: %d)\n", _iCameraMaster);
      return 1;
    }
...
int AndorCameraTest::_retrieveData(int iCamera, uint16_t* liImageData, int iImageWidth, int iImageHeight,
                                   const unsigned char uTestByte, const int iTestBufferSizeIn16,  bool bExtTrigger)
{
  int iError = _selectCamera(iCamera);
  if (iError != 0)
  {
    printf("Failed to switch to camera: %d\n", iCamera);
    return 1;
...
{
  int iError = 0;
  for (int iCamera=0; iCamera<iMaxCamera; ++iCamera)
  {
    iError |= (_closeCamera(iCamera)<<(4*iCamera));
  }
...
int _closeCamera(int iCamera)
{
  ...
  at_32 iCameraHandle;
  iError = GetCameraHandle(iCamera, &iCameraHandle);
  if (!isAndorFuncOk(iError))
  {
    printf("Couldn't get camera handle to close camera: %d\n", iCamera);
    return 1;
  }
  else
  {
    iError = SetCurrentCamera(iCameraHandle);
    if (!isAndorFuncOk(iError))
    {
      printf("Couldn't switch to camera to close: %d\n", iCamera);
      return 2;
    }
  }
  ...
  if (iCameraInitialized[iCamera] != 0)
  {
    ShutDown();
    iCameraInitialized[iCamera] = 0;
  }
  return 0;
}
...
static void showUsage()
{
    printf(
      ...
      "    -h|--help                         Show usage\n"
      "    -c|--camera    <camera number>    Select number of cameras\n"
