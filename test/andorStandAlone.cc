...
static const char sAndorCameraTestVersion[] = "1.40";

namespace PICAM
{
  void printPvError(const char *sPrefixMsg);
  void displayParamIdInfo(int16_t hCam, uint32_t uParamId,
                          const char sDescription[]);
  int getAnyParam(int16_t hCam, uint32_t uParamId, void *pParamValue, int16_t iMode = ATTR_CURRENT);
  int setAnyParam(int16_t hCam, uint32_t uParamId, void *pParamValue);
...
private:     
  // private functions
  int testImageCaptureStandard(char* sFnPrefix, int iNumFrame, int16_t modeExposure, uint32_t uExposureTime);
  int testImageCaptureContinous(int iNumFrame, int16_t modeExposure, uint32_t uExposureTime);  
...
public:  
  static int16_t getHcam() { return _hCam; }
  
private:  
  static int16_t          _hCam;  
};

int16_t ImageCapture::_hCam = -1;
...
{
    printf( "Usage:  andorCameraTest  [-v|--version] [-h|--help] [-c|--camera <camera number>]\n"
...
{
    printf( "Version:  andorCameraTest  Ver %s\n", sAndorCameraTestVersion );
}
...
void signalIntHandler(int iSignalNo)
{
  ...
  int16_t hCam = ImageCapture::getHcam();
  if ( hCam != -1 )
  {
    rs_bool bStatus;
    int16_t   status;
    uint32_t   uNumBytesTransfered;
...
int main(int argc, char **argv)
{
  ...
  while ( int opt = getopt_long(argc, argv, strOptions, loOptions, &iOptionIndex ) )
  {
      ...
      case '?':               /* Terse output mode */
          printf( "andorCameraTest:main(): Unknown option: %c\n", optopt );
          break;
      case ':':               /* Terse output mode */
          printf( "andorCameraTest:main(): Missing argument for %c\n", optopt );
          break;
      default:            
      case 'h':               /* Print usage */
          showUsage();
...
int ImageCapture::updateCameraSettings(int iReadoutPort, int iSpeedIndex, int iGainIndex, int iStrip)
{
  ...
  if ( iStrip >= 0 )
  {
    ...
    int16_t i16Strip = (int16_t) iStrip;
  ...
  if ( iReadoutPort >= 0 )
  {
    ...
    uint32_t u32ReadoutPort = (uint32_t) iReadoutPort;
  ...
  if ( iSpeedIndex >= 0 )
  {
    ...
    int16_t i16SpeedTableIndex = iSpeedIndex;    
  ...
  if ( iGainIndex >= 0 )
  {
    ...
    int16_t i16GainIndex = iGainIndex;    
  ...
  uint32_t u32LogicOutput = (uint32_t) OUTPUT_NOT_SCAN;
...
int ImageCapture::setupCooling(float fTemperature)
{
  ...
  int16_t iCoolingTemp = (int) (fTemperature * 100);
  ...
  int iGainIndex, float fTemperature, int iStrip)
{
  ...
  const int16_t modeExposure    = TIMED_MODE;
  //const int16_t modeExposure = STROBED_MODE;
  const uint32_t u32ExposureTime = iExposureTime;
...
int ImageCapture::testImageCaptureStandard(char* sFnPrefix, int iNumFrame, int16_t modeExposure, uint32_t uExposureTime)
{
...
int ImageCapture::testImageCaptureContinous(int iNumFrame, int16_t modeExposure, uint32_t uExposureTime)
{
  printf( "Starting continuous image capture for %d frames, exposure mode = %d, exposure time = %d ms\n",
    iNumFrame, (int) modeExposure, (int) uExposureTime );
  
  const int16_t iCircularBufferSize = 5;  
  ...
  {    
    ...
    uint32_t uNumBytesTransfered = -1, uNumBufferFilled = -1; 
    int iNumLoop = 0;
...
void printPvError(const char *sPrefixMsg)
{
  ...
  int16_t iErrorCode = pl_error_code();
...
/* ASCII text.                                                                        */
static void displayParamEnumInfo(int16_t hCam, uint32_t uParamId)
{
  boolean status;             /* status of pvcam functions.                */
  uint32_t count, index;         /* counters for enumerated types.            */
...
static void displayParamValueInfo(int16_t hCam, uint32_t uParamId)
{
...
int getAnyParam(int16_t hCam, uint32_t uParamId, void *pParamValue, int16_t iMode)
{
  if (pParamValue == NULL)
  {
