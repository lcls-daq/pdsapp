...
CxiSpectrum::CxiSpectrum(const char*     pvName, 
  ...
  _sem      (Semaphore::FULL),
  _initialized(false),
  _xlo(0),_xhi(200),_ylo(0),_yhi(200), _sinq(0)
{
}

void CxiSpectrum::initialize()
{
  char buff[64];
  ...
  sprintf(buff,"%s:CNTL.H",_pvName);
  _ang_mon  = new PVMonitor(buff,*this);

  _initialized = true;
}

CxiSpectrum::~CxiSpectrum()
{
  if (_initialized) {
...
void   CxiSpectrum::updated()
{
  ...
  { 
    ...
    _sinq = sin(*reinterpret_cast<double*>(_ang_mon->data())*M_PI/180.);
...
void   CxiSpectrum::_configure(const void* payload, const Pds::ClockTime& t) 
{
  updated();
}

void   CxiSpectrum::_event    (const void* payload, const Pds::ClockTime& t) 
{
  if (!_initialized) return;
  ...
  const uint16_t* d = reinterpret_cast<const uint16_t*>(frame.data16().data());
  d += _ylo*frame.width() + _xlo;
  double* p = _spectrum;
  double* q = _pspectrum;
  for(int y=0; y<nb; y++) {
    int v = 0;
    for(int x=int(_xlo); x<int(_xhi); x++) {
      int iy = y + int(double(x-512)*_sinq+0.5);
      if (iy>=0 && iy<1024)
        v += *(d+iy*frame.width() + x)-o;
    }
    *p++ += *q++ = double(v); 
  }
  _nev++;
...
void   CxiSpectrum::_damaged  () {}

void    CxiSpectrum::update_pv() {}

void    CxiSpectrum::_update_pv() 
{
  double* v = reinterpret_cast<double*>(_valu_writer->data());
  if (!v) {
    printf("No PV\n");
