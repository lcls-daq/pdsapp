...
int main(int argc, char** argv)
{
  ...
  while(1) {
    const list<Src>& sources = reader.sources();

    printf("Choices (Ctrl<D> to exit):\n");
    int i=0;
    for(list<Src>::iterator it = sources.begin(); it!=sources.end(); it++,i++) {
      const MonCds& cds = *reader.cds(*it);
      for(unsigned g=0; g<cds.ngroups(); g++) {
	const MonGroup& gr = cds.group(g);
	for(unsigned e=0; e<g.nentries(); e++) {
	  const MonEntry* e = gr->entry(e);
	  printf("%x)  %x.%x.%s\n", 
		 (i<<24) | (g<<16)|e, 
		 it->log(), it->phy(), e->desc().name());
	}
      }
    }
