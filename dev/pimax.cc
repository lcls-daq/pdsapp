...
namespace Pds
{
static const char sPimaxVersion[] = "0.1";

class SegWireSettingsPimax : public SegWireSettings
{
public:
    SegWireSettingsPimax(const Src& src, string sAliasName)
    {
...
//
class EventCallBackPimax : public EventCallback
{
public:
    EventCallBackPimax(int iPlatform, CfgClientNfs& cfgService, int iCamera, bool bDelayMode,
      ...
      _bAttached(false), _pimaxManager(NULL)
    {
    }

    virtual ~EventCallBackPimax()
    {
        reset();
    }
...
private:
    ...
    virtual void attached(SetOfStreams& streams)
    {
        ...
        _pimaxManager->appliance().connect(streamFramework->inlet());
        _bAttached = true;
    }
...
private:
    ...
    int                 _iDebugLevel;
    bool                _bAttached;
    PimaxManager*       _pimaxManager;
}; // class EventCallBackPimax
...
static void showUsage()
{
    printf( "Usage:  pimax  [-v|--version] [-h|--help] [-c|--camera <0-9> ] "
      ...
      "    -g|--config   <db_path>      Intial pimax camera based on the config db at <db_path>\n"
...
{
    printf( "Version:  pimax  Ver %s\n", sPimaxVersion );
}

static int    iSignalCaught   = 0;
static Task*  taskMainThread  = NULL;
void pimaxSignalIntHandler( int iSignalNo )
{
  printf( "\npimaxSignalIntHandler(): signal %d received. Stopping all activities\n", iSignalNo );
  iSignalCaught = 1;
...
int main(int argc, char** argv)
{
    ...
    sigActionSettings.sa_handler = pimaxSignalIntHandler;
    ...
    const DetInfo detInfo( getpid(), detector, iDetectorId, DetInfo::Pimax, iDeviceId);
    ...
    SegWireSettingsPimax settings(detInfo, sUniqueId);

    EventCallBackPimax  eventCallBackPimax(iPlatform, cfgService, iCamera, bDelayMode, bInitTest, sConfigDb, iSleepInt, iDebugLevel);
    SegmentLevel segmentLevel(iPlatform, settings, eventCallBackPimax, NULL);

    segmentLevel.attach();
    if ( eventCallBackPimax.IsAttached() )
